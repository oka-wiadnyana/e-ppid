  ...
  <div class="row">
    <div class="col">
      <h3><i class="fas fa-tachometer-alt mr-2"></i>DAFTAR USER</h3>
    </div>
  </div>
  <div>
    <a href="<?= base_url('admineppid/tambah_user'); ?>" class="btn btn-info mb-2 tambah_link">
  ...
  <script>
    function table_user() {
      let table = $('#data_user').DataTable({
        ...
        "order": [],
        "ajax": {
          "url": "<?= base_url('admineppid/data_user_datatable'); ?>",
          "type": "POST"
        },
        "columnDefs": [{
            "target": 0,
    ...
    $(document).ready(function() {
      ...
      $('#data_user tbody').on('click', 'tr td:nth-child(9) .edit_btn', function(e) {
        e.preventDefault();
        let id = $(this).data('id');
        console.log(id);
        $.ajax({
          ...
          url: "<?= base_url('admineppid/modal_edit_user'); ?>",
